# Find the n-th ranked hospital for a given "outcome" for every state.
#
...
rankall <- function(outcome, num = "best") {
    ...
    # Split the data into per-state data, then find the n-th ranked hospital for
    # each state
    # Drop any lines where the desired outcome is "Not Available". While
    ...
    #
    # For each hospital, extract the hospital name and the 30-day mortality
    # rate for the outcome we're evaluating (we don't need the rest of the
    # data)
    perStateInfo<-split(hospitalData[hospitalData[colNum] != "Not Available",c(hospitalNameCol,colNum)], hospitalData[hospitalData[colNum] != "Not Available",hospitalStateCol])

    hospitals<-lapply(perStateInfo,
                      function(data,num)
                      {
                          if (num == "best")
                          {
                      ...
                      num)
    ranking<-cbind(hospital=hospitals,state=names(hospitals))
    row.names(ranking)<-names(hospitals)
